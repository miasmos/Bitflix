(function($){
	...
	$.fn.youTubeEmbed = function(settings){
		...
		$.get(youtubeAPI,{'q':settings.videoID},function(response){
			...
			/*elements.close = $('<div>',{class:'closeDiv close'})
								.appendTo(elements.container);*/
			...
			window['eventListener_'+settings.safeID] = function(status){
				...
				if(status==-1)	// video is loaded
				{
					if(!initialized)
					{
						...
						/*elements.close.click(function(){
							$('.trailer').fadeOut(200,function(){$(this).remove();});
						});*/
						
						elements.control.click(function(){
							$('.trailer').fadeOut(200,function(event){
								$(this).closest('.movie').trigger('mousenter');
								$(this).remove();
								//$(this).closest('.movie').stop(true).animate({width:"154"},200);
							});
				...
				if(status==0){ // video has ended
					$('.trailer').fadeOut(200,function(){$(this).remove();});
					elements.close.removeClass('close');
